...
/** 公式Instagramでサイトに表示する投稿（URLの /p/XXXXX/ の XXXXX。例: https://www.instagram.com/p/DXeUfhND4sC/ → DXeUfhND4sC） */
...
export default function Home() {
  ...
  const [newsImageLoading, setNewsImageLoading] = useState<
  ...
  const [youtubeEmbedLoading, setYoutubeEmbedLoading] = useState<
    Record<string, boolean>
  >({});
  const [youtubeEmbedTimedOut, setYoutubeEmbedTimedOut] = useState<
    Record<string, boolean>
  >({});
  ...
  const jsonLd = {
    ...
    description:
      "風舞流曲技太鼓・ベースボール・器械体操・フィットネスなど、心を豊かにする身体活動を提供しています。",
    url: SITE_BASE_URL,
  ...
  return (
    <div className="min-h-screen text-white overflow-hidden">
      <Helmet>
        <title>
          ARTIST MOTION - アーティストモーション -
          風舞流曲技太鼓・ベースボール・器械体操・フィットネス
        </title>
        <meta
          name="description"
          content="丹波市・三田市・神戸市北区を中心に活動するARTIST MOTION~アーティストモーション~は、風舞流曲技太鼓・ベースボール・器械体操・フィットネスなど、心を豊かにする身体活動を提供しています。お気軽にお問い合わせください。"
        />
        <meta
          name="keywords"
          content="アーティストモーション,ARTIST MOTION,風舞流曲技太鼓,太鼓,和太鼓,太鼓 教室,曲技太鼓,野球,野球クラブ,野球 教室,ベースボール,ベースボールクラブ,器械体操,体操教室,フィットネス,フィットネスクラス,丹波市,丹波,三田市,三田,筋トレ,ダイエット"
        />
        <link rel="canonical" href={`${SITE_BASE_URL}/`} />
        <link
          rel="preload"
          as="image"
          href="/images/koen_oshirase/背景画像.jpg"
        />
        <meta property="og:url" content={`${SITE_BASE_URL}/`} />
        <meta
          property="og:title"
          content="ARTIST MOTION - アーティストモーション"
        />
      ...
      {/* ===== 公演のお知らせセクション ===== */}
      {/* koen-parallax-section::before が position:fixed で背景を固定（iOS Safari 対応） */}
